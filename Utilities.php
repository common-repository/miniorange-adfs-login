...
class Utilities {
    ...
    public static function checkSign($certFingerprint, $signatureData, $certNumber, $relayState) {
        ...
        if (count($certificates) === 0) {
            $storedCerts = maybe_unserialize(get_option('mo_adfs_sso_saml_x509_certificate'));
    ...
    {
        $xml = new DOMDocument();
        $url = plugins_url().'/miniorange-adfs-login/sp-metadata.xml';

        $xml->load($url);

        $xpath = new DOMXPath($xml);
        $elements = $xpath->query('//md:EntityDescriptor[@entityID="http://{path-to-your-site}/wp-content/plugins/miniorange-adfs-login/"]');

         if ($elements->length >= 1) {
            $element = $elements->item(0);
            $element->setAttribute('entityID', $siteUrl.'/wp-content/plugins/miniorange-adfs-login/');
        }
        ...
        //re-save
        $xml->save(plugins_url()."/miniorange-adfs-login/sp-metadata.xml");
    }
    ...
    public static function mo_adfs_wp_remote_post($url, $args = array()){
		$response = wp_remote_post($url, $args);
		if(!is_wp_error($response)){			
			return $response;
		} else {
            $show_message = new saml_mo_login();
            update_option('mo_adfs_sso_message', 'Unable to connect to the Internet. Please try again.');
            $show_message->mo_adfs_show_error_message();
        }
    }
    
    public static function mo_adfs_wp_remote_get($url, $args = array()){
