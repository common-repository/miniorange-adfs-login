...
class mo_login_wid extends WP_Widget {
	public function __construct() {
		$identityName = get_option('mo_adfs_sso_saml_identity_name');
		parent::__construct(
	 		'Saml_Login_Widget',
	...
	public function loginForm(){
		...
		$identity_provider = get_option('mo_adfs_sso_saml_identity_name');
		$saml_x509_certificate=get_option('mo_adfs_sso_saml_x509_certificate');
		...
		if( ! $this->mo_adfs_check_empty_or_null_val(get_option('mo_saml_redirect_error_code')))
		{
		...
		?>
		<?php echo $link_with_username;?> | <a href="<?php echo wp_logout_url(mo_adfs_get_current_page_url()); ?>" title="<?php _e('Logout','mosaml');?>"><?php _e('Logout','mosaml');?></a></li>
		<?php
		}
	}

	public function mo_adfs_check_empty_or_null_val( $value ) {
...
function mo_login_validate(){
	...
	if((isset($_REQUEST['option']) && $_REQUEST['option'] == 'saml_user_login') || (isset($_REQUEST['option']) && $_REQUEST['option'] == 'testConfig') || (isset($_REQUEST['option']) && $_REQUEST['option'] == 'getsamlrequest')|| (isset($_REQUEST['option']) && $_REQUEST['option'] == 'getsamlresponse')){
	    ...
	    if(mo_adfs_is_sp_configured()) {
			if($_REQUEST['option'] == 'testConfig')
				...
				$sendRelayState = mo_adfs_get_current_page_url();
		    ...
		    if( $sendRelayState == 'displaySAMLRequest' )
			    mo_adfs_show_SAML_log(Utilities::createSAMLRequest($acsUrl, $sp_entity_id, $ssoUrl, $force_authn),$sendRelayState);
			$redirect = $ssoUrl;
	...
	if( array_key_exists('SAMLResponse', $_POST) && !empty($_POST['SAMLResponse']) ) {
		...
		if($relayState=='displaySAMLResponse'){
			mo_adfs_show_SAML_log($samlResponse,$relayState);
		}
		...
		$certFromPlugin = maybe_unserialize(get_option('mo_adfs_sso_saml_x509_certificate'));
		...
		if(empty($assertionSignatureData) && empty($responseSignatureData) ) {
			...
			mo_adfs_download_logs($Error_message,$Cause_message);
...
//		checking for certificates from given list
		if(is_array($certFromPlugin)) {
				foreach ($certFromPlugin as $key => $value) {
					...
					$certfpFromPlugin = mo_adfs_convert_to_windows_iconv($certfpFromPlugin);
					/* remove whitespaces */
		...
		$issuer = get_option('mo_adfs_sso_saml_issuer');
		$spEntityId = get_option('mo_saml_sp_entity_id');
		if(empty($spEntityId)) {
			$spEntityId = $sp_base_url.'/wp-content/plugins/miniorange-adfs-login/';
		}
		...
		mo_adfs_checkMapping($attrs,$relayState,$sessionIndex);
	}
	...
	if( isset( $_REQUEST['option'] ) and strpos( $_REQUEST['option'], 'readsamllogin' ) !== false ) {
		...
		if(isset($_POST['STATUS']) && $_POST['STATUS'] == 'ERROR')
		{
			...
			try {
				...
				$firstName = get_option('mo_adfs_sso_first_name');
				$lastName = get_option('mo_adfs_sso_last_name');
				$groupName = get_option('saml_am_group_name');
				$defaultRole = get_option('mo_adfs_sso_default_user_role');
				...
				$key = get_option('mo_adfs_sso_customer_token');
			...
			mo_adfs_login_user($user_email,$firstName,$lastName,$userName, $groupArray, $dontAllowUnlistedUserRole, $defaultRole,$redirect_to, $checkIfMatchBy);
		}

	}
}

function mo_adfs_checkMapping($attrs,$relayState,$sessionIndex){
	try {
		//Get enrypted user_email
		$emailAttribute = get_option('saml_am_email');
		$usernameAttribute = get_option('saml_am_username');
		...
		if($relayState=='testValidate'){
			...
			mo_adfs_show_test_result($firstName,$lastName,$user_email,$groupName,$attrs);
		}else{
			mo_adfs_login_user($user_email, $firstName, $lastName, $userName, $groupName, $dontAllowUnlistedUserRole, $defaultRole, $relayState, $checkIfMatchBy, $sessionIndex, $attrs['NameID'][0]);
		}
...
function mo_adfs_show_test_result($firstName,$lastName,$user_email,$groupName,$attrs){
	if(ob_get_contents())
		ob_end_clean();
	echo '<div style="font-family:Calibri;padding:0 3%;">';
	if(!empty($user_email)){
 ...
 *
 */
function mo_adfs_convert_to_windows_iconv($certfpFromPlugin){
...
function mo_adfs_login_user($user_email, $firstName, $lastName, $userName, $groupName, $dontAllowUnlistedUserRole, $defaultRole, $relayState, $checkIfMatchBy, $sessionIndex = '', $nameId = ''){
	$user_id = null;
	...
	elseif ( username_exists( $userName ) && !email_exists( $user_email ) ){
	 ...
	 mo_adfs_add_firstlast_name($user_id,$firstName,$lastName,$relayState);


}

function mo_adfs_add_firstlast_name($user_id,$first_name,$last_name,$relay_state){
	if( !empty($first_name) )
	{
...
function mo_adfs_show_SAML_log($samlRequestXML,$type){
...
function mo_adfs_get_current_page_url() {
...
function mo_adfs_register_widget(){
	register_widget('mo_login_wid');
}

add_action( 'widgets_init', 'mo_adfs_register_widget' );
