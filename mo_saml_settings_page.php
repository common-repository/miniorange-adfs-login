...
function mo_register_saml_sso() {
    if ( isset( $_GET['tab'] ) ) {
        $active_tab = $_GET['tab'];
    }else if ( mo_adfs_is_customer_registered_saml() ) {
        $active_tab = 'save';
    } else {
        $active_tab = 'login';
    }
    ...
    if ( ! mo_adfs_is_openssl_installed() ) {
        ?>
    ...
    <div id="mo_saml_settings" >
        ...
        <div class="wrap">
            <h1>
                ...
                <?php if($active_tab == 'licensing' || (isset($_REQUEST['page']) && $_REQUEST['page'] == 'mo_saml_licensing')){ ?>
                    ...
                    <div style="float:left;"><a  class="add-new-h2 add-new-hover" style="font-size: 16px; color: #000;" href="<?php echo mo_adfs_add_query_arg( array( 'tab' => 'save' ), htmlentities( $_SERVER['REQUEST_URI'] ) ); ?>"><span class="dashicons dashicons-arrow-left-alt" style="vertical-align: bottom;"></span> Back To Plugin Configuration</a></div>
        ...
        <div class="miniorange_container" id="container">
                ...
                <tr>
                    ...
                    <td style="vertical-align:top;width:65%;">
                        <?php
                        if ( $active_tab == 'save' ) {
                            mo_adfs_apps_config_saml();
                        } else if ( $active_tab == 'opt' ) {
                            mo_adfs_save_optional_config();
                        }   else if ( $active_tab == 'config' ) {
                            mo_adfs_configuration_steps();
                        } else if ( $active_tab == 'general' ) {
                            mo_adfs_general_login_page();
                        } else if($active_tab == 'addons'){
                            mo_adfs_show_addons_page();
                            ...
                            if(mo_adfs_is_customer_registered_saml(false)){
                                mo_adfs_show_customer_details();
                            }else{
                                if ( get_option( 'mo_adfs_sso_verify_customer' ) == 'true' ) {
                                    mo_adfs_show_verify_password_page_saml();
                                }else{
                                    mo_adfs_show_new_registration_page_saml();
                                }
                            }
                        }else {
                                mo_adfs_apps_config_saml();
        ...
        function mo_adfs_is_curl_installed() {
        ...
        function mo_adfs_is_openssl_installed() {
        ...
        function mo_adfs_is_dom_installed(){
        ...
        function mo_adfs_is_iconv_installed(){
        ...
        function mo_adfs_get_attribute_mapping_url(){

            return add_query_arg( array('tab' => 'opt'), $_SERVER['REQUEST_URI'] );
        }

        function mo_adfs_show_customer_details(){
            ?>
            <div class="mo_saml_table_layout" >
                ...
                <tr>
                    ...
                    <td style="width:55%; padding: 10px;"><?php echo get_option( 'mo_adfs_sso_admin_email' ); ?></td>
                </tr>
                <tr>
                    <td style="width:45%; padding: 10px;">Customer ID</td>
                    <td style="width:55%; padding: 10px;"><?php echo get_option( 'mo_adfs_sso_admin_customer_key' ) ?></td>
                </tr>
                </table>
                <br /><br />
        ...
        function mo_adfs_show_new_registration_page_saml() {
            update_option( 'mo_adfs_sso_new_registration', 'true' );
                        ...
                        <table class="mo_saml_settings_table">
                            <tr>
                                ...
                                <td><input class="mo_saml_table_textbox" type="email" name="email"
                                           ...
                                           value="<?php echo ( get_option( 'mo_adfs_sso_admin_email' ) == '' ) ? get_option( 'admin_email' ) : get_option( 'mo_adfs_sso_admin_email' ); ?>"/>
                                </td>
                            </tr>
                            <tr>
        ...
        function mo_adfs_show_verify_password_page_saml() {
            ?>
            <!--Verify password with miniOrange-->
            <form name="f" method="post" action="">
            <?php wp_nonce_field("mo_adfs_sso_verify_customer");?>
                <input type="hidden" name="option" value="mo_adfs_sso_verify_customer"/>
                <div class="mo_saml_table_layout">
                    ...
                    <div id="panel1">
                        ...
                        <table class="mo_saml_settings_table">
                            <tr>
                                ...
                                <td><input class="mo_saml_table_textbox" type="email" name="email"
                                           required placeholder="person@example.com"
                                           value="<?php echo get_option( 'mo_adfs_sso_admin_email' ); ?>"/></td>
                            </tr>
                            <tr>
                                <td><b><font color="#FF0000">*</font>Password:</b></td>
                                <td><input class="mo_saml_table_textbox" required type="password"
            ...
            <?php wp_nonce_field("mo_adfs_forgot_password_form_option");?>
                <input type="hidden" name="option" value="mo_adfs_forgot_password_form_option"/>
            </form>
            <script>
                jQuery('#mo_saml_goback').click(function () {
                    jQuery('#mo_saml_goback_form').submit();
                });
                jQuery("a[href=\"#mo_adfs_forgot_password_link\"]").click(function () {
...
function mo_adfs_general_login_page() {

    ?>
    <?php if ( mo_adfs_is_customer_registered_saml() ) { ?>
            ...
            <div style="margin:2% 0 2% 17px;">
                ...
                <div id="mo_saml_add_widget_steps">
                    <ol>
                        ...
                        <li>Select "Login with <?php echo get_option( 'mo_adfs_sso_saml_identity_name' ); ?>". Drag and drop to your
        ...
        <div style="background-color:#FFFFFF; border:1px solid #CCCCCC; padding:0px 2% 0px 2%;position: relative" id="miniorange-auto-redirect">
            <h3>Option 2: Auto-Redirection from site<sup style="font-size: 12px;">[Available in <a
                        href="<?php echo admin_url( 'admin.php?page=mo_adfs_sso_settings&tab=licensing' ); ?>">Standard, Premium and Enterprise</a> plans]</sup></h3>
        ...
        <div style="background-color:#FFFFFF; border:1px solid #CCCCCC; padding:0px 2% 0px 2%;" >
            ...
            <h3>Option 4: Use a ShortCode<sup style="font-size: 12px;">[Available in <a
                        ...
                        <input type="checkbox" style="background: #DCDAD1;"
                           disabled <?php if ( ! mo_adfs_is_sp_configured() )
            ...
            <div style="display:block;text-align:center;margin:2%;">
                <input type="button"
                       onclick="window.location.href='<?php echo wp_logout_url( site_url() ); ?>'" <?php if ( ! mo_adfs_is_sp_configured() )
                ...
                <span style="color:red;">*</span>These options are configurable in the <a
                        href="<?php echo admin_url( 'admin.php?page=mo_adfs_sso_settings&tab=licensing' ); ?>"><b>standard,
...
function mo_adfs_configuration_steps() {
    $sp_base_url = site_url();
    $sp_entity_id = get_option('mo_saml_sp_entity_id')?:$sp_base_url.'/wp-content/plugins/miniorange-adfs-login/';
    ?>
            ...
            echo '<p style="text-align: center;font-size: 13pt;font-weight: bold;">OR</p>';?>
                ...
                <div style="font-size: 13px;position: relative;background-color: white;border-radius: 5px;padding-left: 13px;padding-right:13px;padding-bottom:13px;" id="metadata_manual">
                    <h4>Link to Configure the Plug in:
                        <a href="https://www.miniorange.com/step-by-step-guide-to-set-up-adfs-as-idp-for-wordpress" target='_blank'>Click Here to
...
function mo_adfs_apps_config_saml() {
    ...
    if ( isset( $_GET['action'] ) && $_GET['action'] == 'upload_metadata' ) {
                ...
                <td colspan="3">
                    <h3>Upload IDP Metadata
                        <span style="float:right;margin-right:25px;">
                            <a href="' . admin_url() . 'admin.php?page=mo_adfs_sso_settings&tab=save' . '"><input type="button" class="button" value="Cancel"/></a>
                        </span>
                    </h3>
                </td>
            </tr>
        ...
        echo '
            <form name="saml_form" method="post" action="' . admin_url() . 'admin.php?page=mo_adfs_sso_settings&tab=save' . '" enctype="multipart/form-data">


        <tr>
                <td width="30%"><strong>Identity Provider Name<span style="color:red;">*</span>:</strong></td>
                <td><input type="text" name="saml_identity_metadata_provider" placeholder="Identity Provider name ADFS" style="width: 100%;" value="ADFS" required /></td>
                </tr>
        ...
        //Broker Service
        $saml_identity_name    = get_option( 'mo_adfs_sso_saml_identity_name' );
        $saml_login_url        = get_option( 'mo_adfs_sso_saml_login_url' );
        $saml_issuer           = get_option( 'mo_adfs_sso_saml_issuer' );
        $saml_x509_certificate = maybe_unserialize( get_option( 'mo_adfs_sso_saml_x509_certificate' ) );
        $saml_x509_certificate = ! is_array( $saml_x509_certificate ) ? array( 0 => $saml_x509_certificate ) : $saml_x509_certificate;
        $saml_response_signed  = get_option( 'mo_adfs_sso_saml_response_signed' );


        $saml_is_encoding_enabled = get_option('mo_saml_encoding_enabled')!==false?get_option('mo_saml_encoding_enabled'):'checked';
            $action = "https://www.miniorange.com/step-by-step-guide-to-set-up-adfs-as-idp-for-wordpress";

        if ( $saml_response_signed == null ) {
            $saml_response_signed = 'checked';
        }
        $saml_assertion_signed = get_option( 'mo_adfs_sso_saml_assertion_signed' );
        if ( $saml_assertion_signed == null ) {
            $saml_assertion_signed = 'Yes';
        }

        $idp_config = get_option( 'mo_adfs_sso_idp_config_complete' );
        ?>
        ...
        <form width="98%" border="0"
            ...
            <table style="width:100%;">
                <tr>
                    ...
                    <td>
                        <h3>Configure ADFS as IDP</h3></td>
                        <td colspan="2"><span style="position: relative;padding-bottom: 14px;padding-top:14px;background: white;border-radius: 10px; float:right;" id="upload-metadata" >

                            <a href="<?php echo admin_url(); ?>admin.php?page=mo_adfs_sso_settings&tab=save&action=upload_metadata" style="margin-left: 15px">

                            <input type="button" class="button button-primary button-large"
                                            value="Upload IDP Metadata"
                                        <?php
                                        if ( ! mo_adfs_is_customer_registered_saml() ) {
                                            echo "disabled";
                                        }
                                        ?>
                ...
                <?php } ?>
                <?php if ( ! $idp_config && mo_adfs_is_customer_registered_saml() ) { ?>
                    <!--<tr>
                ...
                <tr>
                    ...
                    <td><input type="text" name="saml_identity_name"
                               placeholder="Enter Display name to be shown on login"
                               style="width: 95%;" value="<?php echo $saml_identity_name;?>"
                               required <?php if ( ! mo_adfs_is_customer_registered_saml() )
                            echo 'disabled' ?> title="Only alphabets, numbers and underscore is allowed"/></td>
                    <td>

                            <a href="https://www.miniorange.com/step-by-step-guide-to-set-up-adfs-as-idp-for-wordpress" target="_blank" style="margin-left: 15px">

                            <input type="button" class="button button-primary button-large"
                                            value="Click here for guide"
                                        <?php
                                        if ( ! mo_adfs_is_customer_registered_saml() ) {
                                            echo "disabled";
                                        }
                ...
                <tr>
                    ...
                    <td><br/><input type="submit" name="submit" style="width:150px;margin-right: 3%;" value="Save"
                                    class="button button-primary button-large"<?php if ( ! mo_adfs_is_customer_registered_saml() )
                            echo 'disabled' ?>/>
                        <input type="button" id="test_config"
                               title= "<?php if(!mo_adfs_is_openssl_installed()){
                            ...
                            onclick="showTestWindow();" <?php if ( ! mo_adfs_is_sp_configured() || ! get_option( 'mo_adfs_sso_saml_x509_certificate' ) || !mo_adfs_is_openssl_installed() )
...
<!--                        --><?php //if ( ! mo_adfs_is_sp_configured() || ! get_option( 'saml_x509_certificate' ) ) {
//                            echo 'disabled';
...
<!--                </tr>-->
                <tr>
                    ...
                    <td><br /><input type="button" name="saml_request" id="export-import-config"
                        ...
                        <?php if ( ! mo_adfs_is_sp_configured() || ! get_option( 'mo_adfs_sso_saml_x509_certificate' ) ) {
                            echo 'disabled';
        ...
        <script>
            function showTestWindow() {
                var myWindow = window.open("<?php echo mo_adfs_get_test_url(); ?>", "TEST SAML IDP", "scrollbars=1 width=800, height=600");
            }

            function showSAMLRequest() {
                var myWindow = window.open("<?php echo mo_adfs_get_saml_request_url();?>", "VIEW SAML REQUEST", "scrollbars=1 width=800, height=600");
            }

            function showSAMLResponse() {
                var myWindow = window.open("<?php echo mo_adfs_get_saml_response_url();?>", "VIEW SAML RESPONSE", "scrollbars=1 width=800, height=600");
            }


            function redirect_to_attribute_mapping(){
				window.location.href= "<?php echo mo_adfs_get_attribute_mapping_url(); ?>";
			}
        </script>
        <?php
    }
...
function mo_adfs_save_optional_config() {
    global $wpdb;
    $entity_id = get_option( 'entity_id' );
    if ( ! $entity_id ) {
        $entity_id = 'https://auth.miniorange.com/moas';
    ...
    $saml_am_first_name = get_option( 'mo_adfs_sso_first_name' );
    $saml_am_last_name  = get_option( 'mo_adfs_sso_last_name' );
    ...
    <form name="saml_form_am" method="post" action="">
        ...
        <table width="98%" border="0"
            ...
            <?php } ?>
            <tr>
                <td colspan="2">[ <a href="https://docs.miniorange.com/documentation/saml-handbook/attribute-role-mapping/attribute-mapping" id="attribute_mapping" target="_blank    ">Click Here</a> to know how this is useful. ]
            ...
            <tr>
                ...
                <td><input type="text" disabled name="saml_am_first_name" placeholder="Enter attribute name for First Name"
                           ...
                           value="<?php echo $saml_am_first_name; ?>" <?php if ( ! mo_adfs_is_customer_registered_saml() )
                        echo 'disabled' ?>/></td>
            </tr>
            <tr>
                ...
                <td><input type="text" disabled name="saml_am_last_name" placeholder="Enter attribute name for Last Name"
                           style="width: 350px;background: #DCDAD1;"
                           value="<?php echo $saml_am_last_name; ?>" <?php if ( ! mo_adfs_is_customer_registered_saml() )
                        echo 'disabled' ?>/></td>
            </tr>
                ...
                <tr>
                    ...
                    Customized Attribute Mapping is
                        configurable in the <a
                                href="<?php echo admin_url( 'admin.php?page=mo_adfs_sso_settings&tab=licensing' ); ?>"><b>premium
            ...
            <tr>
                ...
                <td>
                    ...
                    <select id="saml_am_default_user_role" name="saml_am_default_user_role" <?php echo $disabled ?>
                            style="width:150px;">
                        <?php
                        $default_role = get_option( 'mo_adfs_sso_default_user_role' );
                ...
                <tr>
                    <td colspan="2"><br/><span style="color:red;">*</span>Customized Role Mapping options are
                        configurable in the <a
                                ...
                                href="<?php echo admin_url( 'admin.php?page=mo_adfs_sso_settings&tab=licensing' ); ?>"><b>standard</b></a>
...
function mo_adfs_is_customer_registered_saml($check_guest=true) {

    $email       = get_option( 'mo_adfs_sso_admin_email' );
    $customerKey = get_option( 'mo_adfs_sso_admin_customer_key' );

    if(mo_adfs_is_guest_enabled() && $check_guest)
        return 1;
...
function mo_adfs_is_guest_enabled(){
...
function mo_adfs_is_sp_configured() {
    $saml_login_url = get_option( 'mo_adfs_sso_saml_login_url' );
...
function mo_adfs_download_logs($error_msg,$cause_msg) {
...
function miniorange_support_saml() {
                ...
                <table class="mo_saml_settings_table">
                    <tr>
                        <td><input style="width:95%" type="email" class="mo_saml_table_textbox" required
                                   ...
                                   value="<?php echo get_option( "mo_adfs_sso_admin_email" ); ?>"
                    ...
                    <tr>
                        <td><input type="tel" style="width:95%" id="contact_us_phone"
                                   ...
                                   value="<?php echo get_option( 'mo_adfs_sso_admin_phone' ); ?>"
                                   placeholder="Enter your phone"></td>
                    </tr>
                    <tr>
                        <td><textarea class="mo_saml_table_textbox" style="width:95%"
                                      onkeypress="mo_adfs_valid_query(this)" onkeyup="mo_adfs_valid_query(this)"
                                      onblur="mo_adfs_valid_query(this)" required name="mo_saml_contact_us_query"
    ...
    <?php miniorange_keep_configuration_saml_adfs(); ?>

    <script>
        jQuery("#contact_us_phone").intlTelInput();
        jQuery("#phone_contact").intlTelInput();

        function mo_adfs_valid_query(f) {
...
function mo_adfs_display_attrs_list(){
...
function miniorange_demo_request_saml(){?>
    ...
    <?php wp_nonce_field("mo_saml_demo_request_option");?>
        ...
        <td><input type="text" name="mo_saml_demo_email" placeholder="We will use this email to setup the demo for you" required style="width:350px" value="<?php echo !empty(get_option('mo_adfs_sso_admin_email')) ? get_option('mo_adfs_sso_admin_email') : ''; ?>"></td>
        </tr>
        ...
        <td><select name="mo_saml_demo_plan" id="mo_saml_demo_plan" style="width:350px" required onchange="mo_adfs_show_description();">
    ...
    <script type="text/javascript">
        function mo_adfs_show_description() {
...
function mo_adfs_get_saml_request_url() {
...
function mo_adfs_get_saml_response_url() {
...
function mo_adfs_add_query_arg($query_arg, $url){
    if(strpos($url, 'mo_saml_licensing') !== false){
        $url = str_replace('mo_saml_licensing', 'mo_adfs_sso_settings', $url);
    }
